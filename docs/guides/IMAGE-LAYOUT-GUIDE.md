...
**❌ WRONG - Relative paths (don't use these in actual content files):**
```markdown
...
**Note:** This documentation shows `../../` in some references for navigation between documentation files, but in actual content files (_projects/, _builds/, etc.) always use absolute paths starting with `/assets/`.
