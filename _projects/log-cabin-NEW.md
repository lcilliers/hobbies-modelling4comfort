...
.chapter-section h2 {
  ...
  border-bottom: 1px solid #97c2a2;
}
...
/* Image layout styles */
.build-image, .build-image-medium, .build-image-large {
  margin: 2rem 0;
}

.build-image img, 
.build-image-medium img, 
.build-image-large img {
  width: 100%;
  height: auto;
  border-radius: 4px;
...
.build-image-medium figcaption,
.build-image-large figcaption {
  font-size: 0.9rem;
  color: #666;
  font-style: italic;
  margin-top: 0.5rem;
...
/* Special handling for small figure image */
.figure-small {
  max-width: 400px;
  margin-left: auto;
  margin-right: auto;
...
/* Gallery grid (from existing styles, enhanced) */
.project-gallery {
  display: grid;
